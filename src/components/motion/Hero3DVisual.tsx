...
const ProgramThumb = ({
  ...
  <div className={`hero-bento-thumb ${area}`}>
...
const Hero3DVisual = () => {
  ...
  return (
    <div
      ref={ref}
      className="scene-3d relative mx-auto w-full max-w-xl lg:max-w-none"
      onMouseMove={onMouseMove}
      onMouseLeave={onMouseLeave}
    >
      <div className="hero-image-glow scale-105" aria-hidden />

      <div className="hero-bento-stage" style={style}>
        {/* Side program cards — each in its own grid cell, fully visible */}
        {sidePrograms.map((p) => (
          <ProgramThumb key={p.label} {...p} />
        ))}

        {/* Center — main hero */}
        <div className="hero-bento-main bento-main">
          <img
        ...
        {/* Stats row below grid on mobile, inline on desktop */}
        <div className="bento-stats flex flex-wrap items-center justify-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-1.5 rounded-xl border border-secondary/30 bg-secondary px-3 py-2 text-white shadow-md">
            <Zap className="h-4 w-4" />
            <div>
              <p className="text-xs font-bold leading-none">85%</p>
              <p className="text-[9px] text-white/80">Growth</p>
            </div>
          </div>
        </div>
      </div>
