...
const DevOpsHeroVisual = ({ image, imageAlt }: DevOpsHeroVisualProps) => (
  <div className="devops-hero-visual scene-3d relative mx-auto w-full max-w-lg lg:max-w-none">
    ...
    <DepthCard className="devops-hero-main-card" maxTilt={6}>
      <img src={image} alt={imageAlt} className="h-full w-full object-cover" loading="eager" />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-brand-950/80 via-brand-950/15 to-primary/5" />
      <div className="devops-hero-scan" aria-hidden />
      ...
      <div className="absolute bottom-0 left-0 right-0 flex flex-wrap items-end justify-between gap-2 p-4">
        <div className="rounded-xl border border-white/15 bg-white/10 px-3 py-2 backdrop-blur-md">
          ...
          <p className="mt-0.5 text-[10px] text-white/75 sm:text-xs">Production-ready pipelines</p>
        </div>
